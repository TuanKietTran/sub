...
export class SocialDate extends ValueObject<SocialDateProps> {
   ...
   static of(year: number, month: number, day: number): SocialDate {
     ...
     get day(): number { return this.props.day }
     ...
     private compareTo(other: SocialDate): number {
       if (this.props.year !== other.props.year) return this.props.year - other.props.year
       if (this.props.month !== other.props.month) return this.props.month - other.props.month
     ...
     plusDays(days: number): SocialDate {
       ...
       // use UTC to avoid DST shifting a day
       const utc = Date.UTC(this.props.year, this.props.month - 1, this.props.day)
     ...
     plusMonths(months: number): SocialDate {
       ...
       // clamp day if target month shorter (Jan 31 + 1 month → Feb 28/29)
     ...
     daysUntil(other: SocialDate): number {
       ...
       const b = Date.UTC(other.props.year, other.props.month - 1, other.props.day)
   ...
   // helper — not exported, not a method (no need for `this`)
